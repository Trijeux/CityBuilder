...
void UiButton::CreateButton(const sf::Vector2f pos, const std::string& text, const int character_size, const sf::Color color_text)
{
    ...
    sprite_.setTexture(texture_);  // Set the texture for the button's sprite
    sprite_.setOrigin(sprite_.getGlobalBounds().width / 2.0f, sprite_.getGlobalBounds().height / 2.0f);  // Set the origin of the sprite to its center
